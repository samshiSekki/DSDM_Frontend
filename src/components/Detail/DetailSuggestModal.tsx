...
const DetailSuggestModal = ({
      ...
      if (clubID == 0) {
        try {
          const response = await axios.post(
            `${process.env.REACT_APP_BASE_URL}${process.env.REACT_APP_SUGGESTION}`,
            {
              content: suggestMsg,
              name: "",
            }
        ...
        try {
          const response = await axios.post(
            `${process.env.REACT_APP_BASE_URL}${process.env.REACT_APP_SUGGESTION}/${clubID}`,
            {
              content: suggestMsg,
            }
          );
  ...
  const messageChanged = (e: any) => {
    ...
    e.target.value == "" ? setSendActive(false) : setSendActive(true);
  };
      ...
      >
        <div>
          <BrowserView>
            <textarea
              css={suggestTextArea}
              placeholder="수정/변경 사항 요청이나 신규 동아리 추가 제안 등의 제안할 내용을 적어주세요."
              onChange={(e) => {
                messageChanged(e);
              }}
            />
            ...
            <div
              css={sendActive ? sendBtnActivated : sendBtn}
              onClick={sendSuggest}
            >
              &gt; 보내기
            </div>
          ...
          <MobileView>
            ...
            <div
              css={sendActive ? mobileSendBtnActivated : mobileSendBtn}
              onClick={sendSuggest}
            >
              &gt; 보내기
            </div>
